...
/*
this file is taken from https://github.com/ente-io/frame/blob/bfff1966a455b7108c7365b9ad8aafa479309025/lib/utils/crypto_util.dart
 ...
 */
  return EncryptionResult(
      encryptedData: encryptedData, header: initPushResult.header, key: new Uint8List(0), nonce: new Uint8List(0));
}

Future<EncryptionResult> chachaEncryptFileOwn(Map<String, dynamic> args) async {
  final encryptionStartTime = DateTime.now().millisecondsSinceEpoch;
  // todo final logger = Logger("ChaChaEncrypt");
  print('* LOG * ChaChaEncrypt');
  final sourceFile = io.File(args["sourceFilePath"]);
  final destinationFile = io.File(args["destinationFilePath"]);
  final sourceFileLength = await sourceFile.length();
  // todo logger.info("Encrypting file of size " + sourceFileLength.toString());
  print('* LOG * Encrypting file of size ' + sourceFileLength.toString());
  print('* LOG * Sodium.cryptoSecretstreamXchacha20poly1305Abytes length: ' + Sodium.cryptoSecretstreamXchacha20poly1305Abytes.toString());
  final inputFile = sourceFile.openSync(mode: io.FileMode.read);
  final key = args["key"] ?? Sodium.cryptoSecretstreamXchacha20poly1305Keygen();
  final initPushResult =
  Sodium.cryptoSecretstreamXchacha20poly1305InitPush(key);
  // todo new: store header = nonce in file
  await destinationFile.writeAsBytes(initPushResult.header, mode: io.FileMode.append);
  print('* LOG * initPushResult.header stored in destinationFile');
  // todo remove
  print('* LOG * initPushResult.header length ' + initPushResult.header.length.toString());
  var bytesRead = 0;
  ...
  // todo logger.info("Encryption time: " + (DateTime.now().millisecondsSinceEpoch - encryptionStartTime).toString());
  print('* LOG * Encryption time: ' + (DateTime.now().millisecondsSinceEpoch - encryptionStartTime).toString());
  // todo geändert
  // return EncryptionResult(key: key, header: initPushResult.header);
  print('* LOG * initPushResult.header: ' + initPushResult.header.toString());
  return EncryptionResult(key: key, header: initPushResult.header, encryptedData: new Uint8List(0), nonce: new Uint8List(0));
}

Future<void> chachaDecryptFileOwn(Map<String, dynamic> args) async {
  // todo final logger = Logger("ChaChaDecrypt");
  print('* LOG * ChaChaDecrypt');
  final decryptionStartTime = DateTime.now().millisecondsSinceEpoch;
  final sourceFile = io.File(args["sourceFilePath"]);
  final destinationFile = io.File(args["destinationFilePath"]);
  final sourceFileLength = await sourceFile.length();
  // todo logger.info("Decrypting file of size " + sourceFileLength.toString());
  print('* LOG * Decrypting file of size ' + sourceFileLength.toString());
  final inputFile = sourceFile.openSync(mode: io.FileMode.read);

  // todo new: read the header direct from file
  final headerSize = Sodium.cryptoSecretstreamXchacha20poly1305Headerbytes;
  print('* LOG * Sodium.cryptoSecretstreamXchacha20poly1305Headerbytes: ' + headerSize.toString());
  final headerFromFile = inputFile.readSync(headerSize);
  final pullState = Sodium.cryptoSecretstreamXchacha20poly1305InitPull(headerFromFile, args["key"]);
  //final pullState = Sodium.cryptoSecretstreamXchacha20poly1305InitPull(args["header"], args["key"]);
  ...
  // todo logger.info("ChaCha20 Decryption time: " + (DateTime.now().millisecondsSinceEpoch - decryptionStartTime).toString());
  print('* LOG * Decryption time: ' + (DateTime.now().millisecondsSinceEpoch - decryptionStartTime).toString());
}
...
return EncryptionResult(
     ...
     */
    return EncryptionResult(
        key: key, nonce: nonce, encryptedData: encryptedData, header: new Uint8List(0));
  }
  ...
  //Future<DerivedKeyResult> deriveSensitiveKey(
    ...
    // todo final logger = Logger("pwhash");
    ...
    int opsLimit = Sodium.cryptoPwhashOpslimitModerate;
    Uint8List key;
